...
- **Clean charges (game.js + pet.js):** the Clean (🫧) action is limited to **3 charges, +1 every 5 min** (`CLEAN_REFILL_MS`, `CLEAN_MAX_CHARGES`), mirroring Heal/Play (`refreshCleanCharges`/`cleanChargeRemaining`, fields `cleanCharges`/`cleanRefillAt`, migrated clamp 0–3). `doClean` refuses at 0 with `toast.cleanNotReady` (next-charge countdown) else consumes a charge (starts the timer if full) and applies the existing bath effect (+75 hygiene / −8 happiness). `#btn-clean` shows `action.cleanCharges` ("Clean {n}/3") and, at 0, disables with `action.cleanCooldown` ("Clean {time}"), updated live in `refreshBars`.
